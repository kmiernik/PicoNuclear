...
This class is based on the following PicoScope5000A from 

gamma-spectroscopy project for PicoScope
    Version: 0.9.3
    Home-page: https://github.com/davidfokkema/gamma-spectroscopy
    Author: David Fokkema
    Author-email: davidfokkema@icloud.com

But it provides an Interface to a 3000a Series PicoScope, and introduces
some minor changes
...
class PicoScope3000A:
    """Interface to a 3000 Series PicoScope.

    This class encapsulates the low-level PicoSDK and offers a python-friendly
    interface to a 3000 Series PicoScope (e.g. a PicoScope 5242D).
    ...
    def open(self, serial=None):
        ...
        status = ps.ps3000aOpenUnit(ctypes.byref(handle), serial)
    ...
    def set_channel(self, channel_name, coupling_type='DC', range_value=1,
        ...
        assert_pico_ok(ps.ps3000aMaximumValue(self._handle,
    ...
    def measure(self, num_pre_samples, num_post_samples, timebase=1,
        ...
        with data values in volts. An array of time values in nanoseconds is
        also returned.
    ...
    def measure_adc_values(self, num_pre_samples, num_post_samples, timebase=1,
                           num_captures=1):
        """Start a data collection run and return the data in ADC values.
    ...
    def get_interval_from_timebase(self, timebase, num_samples=1000):
        ...
        assert_pico_ok(ps.ps3000aGetTimebase2(
            self._handle, timebase, num_samples, ctypes.byref(interval), 1,
            None, 0))
    ...
    def _set_memory_segments(self, num_segments, num_samples):
        ...
        assert_pico_ok(ps.ps3000aMemorySegments(self._handle, num_segments,
    ...
    def start_run(self, num_pre_samples, num_post_samples, timebase=1,
        ...
        assert_pico_ok(ps.ps3000aSetNoOfCaptures(self._handle, num_captures))
        assert_pico_ok(ps.ps3000aRunBlock(
            self._handle, num_pre_samples, num_post_samples, timebase, 1,
            None, 0, callback, None))
    ...
    def _get_values(self, num_samples, num_captures):
        ...
        status = ps.ps3000aGetValuesBulk(
    ...
    def set_trigger(self, channel_name, threshold=0., direction='RISING',
        ...
        :param auto_trigger: the maximum amount of time to wait for a trigger
            before starting capturing data in miliseconds
...
def _get_channel_from_name(channel_name):
    ...
    if channel_name in ['A', 'B', 'C', 'D']:
        def_name = f"PS3000A_CHANNEL_{channel_name}"
    else:
        raise InvalidParameterError(f"Channel {channel_name} is not supported")
    return ps.PS3000A_CHANNEL[def_name]
...
def _get_coupling_type_from_name(coupling_type_name):
    ...
    if coupling_type_name in ['AC', 'DC']:
        def_name = f"PS3000A_{coupling_type_name}"
    else:
        raise InvalidParameterError(f"Coupling type {coupling_type_name} is "
                                    "not supported")
    return ps.PS3000A_COUPLING[def_name]
...
def _get_range_from_value(range):
    ...
    if range in INPUT_RANGES:
        ...
        def_name = f"PS3000A_{range_name}"
    else:
        raise InvalidParameterError(f"Range {range} V is not supported")
    return ps.PS3000A_RANGE[def_name]
